...
__all__ = ['calculate_unstable_score','calculate_activation_mask_act_variance', 'calculate_activation_mask_act_gradient', 'calculate_activation_mask_act_magnitude',
        'mnist_8_200_hook', 'mnist_8_200_hook_pre', 'mnist_2_32_hook', 'mnist_2_32_hook_pre','mnist_1_64_hook', 'mnist_1_64_hook_pre', 'mnist_conv_big_hook', 'mnist_conv_big_hook_pre',
...
def calculate_unstable_score(model_hook, args, Hard_mask=False):
    np.random.seed(1)
    ...
    for idx in range(layer_num):
        ...
        # calculate no. of unstable images determined by absolute difference 
        # between positive and negative activation counts: low difference -> high instability 
        n = layer_act.size(dim = 0)
        assert n == 1000
        pos = torch.where(layer_act >= 0, 1, 0).sum(0)
        score = (n - (2*pos - n).abs())/2
    ...
    if args.global_prune:
        ...
        #added:
        #torch.save(output_mask, os.path.join(args.save_dir,'global_mask_variance_{}_{}_new.pth'.format(args.arch, args.loss)))
    else:        
        print('Generate Layer-wise Mask')
        output_mask = local_mask(new_scores, args.remain_ratio)
        #added:
        #torch.save(output_mask, os.path.join(args.save_dir,'local_mask_variance_{}_{}_new.pth'.format(args.arch, args.loss)))
...
def calculate_activation_mask_act_variance(model_hook, args, Hard_mask=False):
    np.random.seed(1)
    if args.dataset == 'cifar10':
        index = np.random.permutation(50000)[:1000]
        calculate_set = Subset(CIFAR10(args.data, train=True, transform=transforms.ToTensor(), download=True),list(index))
...
def calculate_activation_mask_act_gradient(model_hook, args, Hard_mask=False):
    ...
    if args.global_prune:
        print('Generate Global Mask')
        output_mask = global_mask(new_scores, args.remain_ratio)
        #torch.save(output_mask, os.path.join(args.save_dir,'global_mask_gradient_{}_{}_new.pth'.format(args.arch, args.loss)))
    else:        
        print('Generate Layer-wise Mask')
        output_mask = local_mask(new_scores, args.remain_ratio)
        #torch.save(output_mask, os.path.join(args.save_dir,'local_mask_gradient_{}_{}_new.pth'.format(args.arch, args.loss)))
...
def calculate_activation_mask_act_magnitude(model_hook, args, Hard_mask=False):
    ...
    if args.global_prune:
        ...
        output_mask = global_mask(new_scores, args.remain_ratio)
        #added:
        #torch.save(output_mask, os.path.join(args.save_dir,'global_mask_magnitude_{}_{}_new.pth'.format(args.arch, args.loss)))
    else:        
        print('Generate Layer-wise Mask')
        output_mask = local_mask(new_scores, args.remain_ratio)
        #added:
        #torch.save(output_mask, os.path.join(args.save_dir,'local_mask_magnitude_{}_{}_new.pth'.format(args.arch, args.loss)))
...
class mnist_2_32_hook(nn.Module):
    def __init__(self):
        super(mnist_2_32_hook, self).__init__()

        self.layer0 = Flatten()
        self.layer1 = nn.Linear(784, 32)
        self.layer2 = Relu_Mask(size=(32))
        self.layer3 = nn.Linear(32, 32)
        self.layer4 = Relu_Mask(size=(32))
...
class mnist_2_32_hook_pre(nn.Module):

    def __init__(self):
        super(mnist_2_32_hook_pre, self).__init__()
...
class mnist_1_64_hook(nn.Module):
    def __init__(self):
        super(mnist_1_64_hook, self).__init__()

        self.layer0 = Flatten()
        self.layer1 = nn.Linear(784, 64)
        self.layer2 = Relu_Mask(size=(64))
...
class mnist_1_64_hook_pre(nn.Module):

    def __init__(self):
        super(mnist_1_64_hook_pre, self).__init__()
...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, norm="l_inf", 
    ...
    if cuda_available:
        max_loss = max_loss.cuda()
        max_delta = max_delta.cuda()
