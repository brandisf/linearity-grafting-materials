        ...
        if x.shape[0] == 1:
            ...
            if only_target_attack:
                ...
                if attack_label == pidx_list[0]:
                    ...
                    return True, attack_image.detach(), [attack_margin], attack_image.detach()
                else:
                    print(f"targeted pgd failed, margin {attack_margin}")
                    return False, attack_image.detach(), [attack_margin], attack_image.detach()
            else:
                ...
                if attack_label != y:
                    ...
                    return True, all_targets_attack_image.detach().detach(), attack_margin.detach().cpu().numpy(), attack_image.detach()
                    #return True, attack_image.detach(), [attack_margin]
                else:
                    print("untargeted pgd failed")
                    return False, all_targets_attack_image.detach(), attack_margin.detach().cpu().numpy(), attack_image.detach()
