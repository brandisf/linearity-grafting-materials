...
from model_defs import mnist_2_32, mnist_1_64, mnist_conv_big, mnist_8_200, cifar_conv_big, cifar_cnn_b, cifar_cnn_17m, resnet4b, svhn_conv_big, mnist_conv_big_bn, mnist_8_200_bn
...
def main():
    ...
    if cuda_available:
        model.cuda()
    print(model)
    cnt_parameters(model.state_dict())
...
def train_epoch_adv(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (image, target) in enumerate(train_loader):
        ...
        if epoch < args.warmup:
            warmup_lr(epoch, i+1, optimizer, len(train_loader), args)
...
def cnt_parameters(state_dict):
    num = 0
    for key in state_dict.keys():
        num += state_dict[key].nelement()
    print('* Para = {:.4f} MB'.format(num/2**20))
