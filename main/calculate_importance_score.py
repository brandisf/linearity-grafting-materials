...
def main():
    ...
    if args.mode == 'gradient':
        ...
        mask_score = calculate_activation_mask_act_variance(model_hook, args, Hard_mask=False)
        torch.save(mask_score, os.path.join(args.save_dir,'score_variance_{}_{}_new.pth'.format(args.arch, args.loss)))
    elif args.mode == 'instability':
        mask_score = calculate_unstable_score(model_hook, args, Hard_mask=False)
        torch.save(mask_score, os.path.join(args.save_dir,'score_instability_{}_{}_new.pth'.format(args.arch, args.loss)))
