...
def load_dataset_test():
    ...
    if arguments.Config["data"]["dataset"] == 'MNIST_Test':
        loader = datasets.MNIST
        BS=10000
    elif arguments.Config["data"]["dataset"] == 'CIFAR_Test':
        loader = datasets.CIFAR10
        BS=10000
    elif arguments.Config["data"]["dataset"] == 'SVHN_Test':
        loader = datasets.SVHN
        BS=26032
    else:
        raise ValueError("Dataset {} not supported.".format(arguments.Config["data"]["dataset"]))

    if 'SVHN' in arguments.Config["data"]["dataset"]:
        test_data = loader(database_path, split='test', download=True, transform=transforms.Compose([transforms.ToTensor(), normalize]))
    else:
    ...
    return test_data, data_max, data_min, BS
...
def load_dataset_train():
    ...
    database_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'datasets')
    normalize = transforms.Normalize(mean=arguments.Config["data"]["mean"], std=arguments.Config["data"]["std"])
    if arguments.Config["data"]["dataset"] == 'MNIST_Train':
        loader = datasets.MNIST
        BS=60000
    elif arguments.Config["data"]["dataset"] == 'CIFAR_Train':
        loader = datasets.CIFAR10
        BS=50000
    elif arguments.Config["data"]["dataset"] == 'SVHN_Train':
        loader = datasets.SVHN
        BS=73257
    else:
        raise ValueError("Dataset {} not supported.".format(arguments.Config["data"]["dataset"]))
    ...
    if 'SVHN' in arguments.Config["data"]["dataset"]:
        test_data = loader(database_path, split='train', download=True, transform=transforms.Compose([transforms.ToTensor(), normalize]))
    else:
        test_data = loader(database_path, train=True, download=True, transform=transforms.Compose([transforms.ToTensor(), normalize]))
    test_data.mean = torch.tensor(arguments.Config["data"]["mean"])
    test_data.std = torch.tensor(arguments.Config["data"]["std"])
    # set data_max and data_min to be None if no clip
    data_max = torch.reshape((1. - test_data.mean) / test_data.std, (1, -1, 1, 1))
...
def load_generic_dataset(eps_temp=None):
    ...
    print("Trying generic MNIST/CIFAR/SVHN data loader.")

    if "Train" in arguments.Config["data"]["dataset"]:
        print('Using training set')
        test_data, data_max, data_min, BS = load_dataset_train()

    elif "Test" in arguments.Config["data"]["dataset"]:
        print('Using test set')
        test_data, data_max, data_min, BS = load_dataset_test()

    if eps_temp is None:
        raise ValueError('You must specify an epsilon')
    testloader = torch.utils.data.DataLoader(test_data, batch_size=BS, shuffle=False, num_workers=4)
...
def load_verification_dataset(eps_before_normalization):
    ...
    elif "CIFAR" in arguments.Config["data"]["dataset"] or "MNIST" in arguments.Config["data"]["dataset"] or 'SVHN' in arguments.Config["data"]["dataset"]:
